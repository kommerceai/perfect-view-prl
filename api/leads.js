...
module.exports = async function handler(req, res) {
  if (cors(req, res)) return; // handle preflight
  ...
  try {
    const { firstName, lastName, email, phone, propertyType, service, state, projectDetails, source, tags } = req.body;
    ...
    const contact = await ghl.createOrUpdateContact({
      firstName, lastName, email, phone, propertyType, service, state, projectDetails, source,
    });
    ...
    if (pipeline && stage) {
      await ghl.createOpportunity(contactId, pipeline.id, stage.id, { firstName, lastName, service });
