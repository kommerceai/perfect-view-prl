...
const TAGS = [
  'website-lead', 'free-estimate', 'ghl-calendar', 'phone-inquiry',
  'svc-roofing', 'svc-painting', 'svc-construction', 'svc-landscaping', 'svc-restoration',
  'estimate-scheduled', 'proposal-sent', 'job-active', 'job-complete',
  'reviewed', 'nurture-active', 'nurture-complete', 'do-not-contact', 'long-term-nurture',
];
...
module.exports = async function handler(req, res) {
  ...
  /* ── 2. Custom fields ────────────────── */
  let existingFields = [];
  try { existingFields = await ghl.getCustomFields(); } catch (e) { /* continue */ }
  ...
  /* ── 3. Tags ─────────────────────────── */
  let existingTags = [];
  try { existingTags = await ghl.getTags(); } catch (e) { /* continue */ }
